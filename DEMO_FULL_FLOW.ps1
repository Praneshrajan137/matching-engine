...
function Write-Note { Write-Host "ℹ️  $args" -ForegroundColor Cyan }
...
Write-Step "Test 1: LIMIT Order (Buy @ 60000)"
$limitOrder = @{
    symbol = $symbol
    side = "buy"
    order_type = "limit"
...
$restingOrders = @(
    @{ side="buy"; price="59900"; quantity="0.5" },  # Lower price
    @{ side="buy"; price="60000"; quantity="1.0" },  # Best bid
    @{ side="buy"; price="59800"; quantity="0.3" }   # Even lower
)
...
foreach ($order in $restingOrders) {
    ...
    Write-Demo "Placed: BUY @ $($order.price) x $($order.quantity) -> $($response.order_id)"
...
Write-Note "Placing aggressive SELL order @ 59500 (should match with 60000 bid)..."
$aggressiveOrder = @{
    symbol = $symbol
    side = "sell"
    order_type = "limit"
...
Write-Demo "Placed: SELL @ 59500 x 0.8 -> $($response.order_id)"
...
Write-Title "✅ DEMONSTRATION COMPLETE - ALL REQUIREMENTS SATISFIED"
